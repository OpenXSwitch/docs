...
Pass `userId` and `accountId` directly in the payload, provide an `idempotencyKey`, and designate the specific target blockchain network string (e.g., `ethereum`).
...
## 🔄 What Does This Mean for Existing Integrations?
...
### 👤 1. Users Infrastructure (Zero-Friction User Migration)
...
- **What to Change:** Update the base URL endpoint in your requests—your existing sub-wallet IDs continue to work seamlessly without additional mapping.

### 👛 2. Address & Wallet Management
...
- `accountId`**:** Retrieve your Account ID by completing the workspace setup via the [Accounts & Workspace Console](https://www.google.com/search?q=https://console.openxswitch.com/).
...
### 🔔 4. Webhook Event Consumption

Refer to the complete [Webhook Event Documentation](/webhook/webhook-events) for the updated topic structure and full payload schema reference.
