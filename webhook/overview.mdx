---
title: "Overview"
description: "This guide explains how to configure, test, and secure webhooks for seamless integration with the OpenXSwitch API."
---
...
1. Navigate to **Account → Webhook** on the OpenXSwitch Console.
2. Add your webhook `endpoint` and `secret`.
3. Select the event types you want to subscribe to (e.g., `withdraw`, `deposit`).
4. Choose the environment mode:
   - **Sandbox (default)** - for`testnet` and testing
   - **Live** -`mainnet` events
...
```json Wallet  Deposit highlight={12-13}
{
  "requestId": "",
  "method": "wallets:deposit.pending",
  "params": {
    ...
  }
}
